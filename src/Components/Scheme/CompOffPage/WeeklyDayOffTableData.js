...
export default function WeeklyDayOffTableData() {
  ...
  const [weeklyOffRow, setWeeklyOffRow] = useState(initialValues);
  const [weeklyOffList, setWeeklyOffList] = useState([]);
  const weeklyOffSchema = Yup.object().shape({
    ...
    return (
      <div className="d-flex justify-content-center">
        ...
        <Button
          ...
          onClick={() => {
            const filterData = weeklyOffList.filter(
              data => data.id !== deleteRowId,
            );
            setWeeklyOffList(filterData);
            onHide(name);
          }}
          autoFocus
        />
  ...
  const hanleUpdateData = (index, toggleValue) => {
    let newData = weeklyOffList.map(item => {
      if (item.id === index) {
        return { ...item, isEditable: toggleValue };
      }
      return item;
    });
    setWeeklyOffList(newData);
  };
  ...
  return (
    <div>
      <Formik
        initialValues={weeklyOffRow}
        validationSchema={weeklyOffSchema}
        innerRef={submitRef}
        enableReinitialize
