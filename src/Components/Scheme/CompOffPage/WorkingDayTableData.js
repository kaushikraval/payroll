...
export default function WorkingDayTableData() {
  ...
  const [workingDayRow, setWorkingDayRow] = useState(initialValues);
    ...
    return (
      <div className="d-flex justify-content-center">
        ...
        <Button
          ...
          onClick={() => {
            const filterData = workingDayList.filter(
              data => data.id !== deleteRowId,
            );
            setWorkingDayList(filterData);
            onHide(name);
          }}
          autoFocus
        />
  ...
  const hanleUpdateData = (index, toggleValue) => {
    let newData = workingDayList.map(item => {
      if (item.id === index) {
        return { ...item, isEditable: toggleValue };
      }
      return item;
    });
    setWorkingDayList(newData);
  };
  ...
  return (
    <div>
      <Formik
        initialValues={workingDayRow}
        validationSchema={workingDaySchema}
        innerRef={submitRef}
        enableReinitialize
