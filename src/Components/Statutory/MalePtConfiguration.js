...
export default function MalePtConfiguration() {
  ...
  const [malePtRow, setMalePtRow] = useState(initialValues);
  const [malePtList, setMalePtList] = useState([]);
  const submitRef = useRef(null);
  const malePtConfigurationSchema = Yup.object().shape({
    ...
    return (
      <div className="d-flex justify-content-center">
        ...
        <Button
          ...
          onClick={() => {
            const filterData = malePtList.filter(
              data => data.id !== deleteRowId,
            );
            setMalePtList(filterData);
            onHide(name);
          }}
          autoFocus
        />
  ...
  const hanleUpdateData = (index, toggleValue) => {
    let newData = malePtList.map(item => {
      if (item.id === index) {
        return { ...item, isEditable: toggleValue };
      }
      return item;
    });
    setMalePtList(newData);
  };
  ...
  return (
    <div className="data_table_one lwf_table pt_config_table mb-3">
      <h6>Male</h6>
      <Formik
        initialValues={malePtRow}
        validationSchema={malePtConfigurationSchema}
        innerRef={submitRef}
        enableReinitialize
